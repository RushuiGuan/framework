...
namespace Albatross.Messaging.Commands.Messages {
	public record class CommandErrorReply2 : CommandMessage {
		public static string MessageHeader => "cmd-err2";
		public string ClassName { get; private set; } = string.Empty;
		public byte[] Message { get; private set; } = Array.Empty<byte>();

		public CommandErrorReply2(string route, ulong id, string commandType, string className, byte[] message) : base(MessageHeader, route, id, commandType) {
			ClassName = className;
			Message = message;
		}
		public CommandErrorReply2() { }
