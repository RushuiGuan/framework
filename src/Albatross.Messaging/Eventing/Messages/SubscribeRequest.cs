...
namespace Albatross.Messaging.Eventing.Messages {
	public record class SubscriptionRequest : Message, IMessage {
		...
		public static IMessage Accept(string route, ulong id, NetMQMessage frames) {
			...
			return new SubscriptionRequest(route, id, on, topics);
		}

		public SubscriptionRequest(string route, ulong id, bool on, IEnumerable<string> topics) : base(MessageHeader, route, id) {
			this.On = on;
			Topics = topics;
		}
