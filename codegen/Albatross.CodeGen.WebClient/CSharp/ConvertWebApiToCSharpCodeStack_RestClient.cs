...
namespace Albatross.CodeGen.WebClient.CSharp {
	public class ConvertWebApiToCSharpCodeStack_RestClient : IConvertObject<ControllerInfo, CodeStack> {
		const string ProxyService = "ProxyService";
		private readonly CodeGenSettings settings;

		public ConvertWebApiToCSharpCodeStack_RestClient(CodeGenSettings settings) {
		...
		public CodeStack Convert(ControllerInfo from) {
			...
			using (codeStack.NewScope(new CompilationUnitBuilder())) {
				codeStack
					...
					.With(new UsingDirectiveNode("Albatross.RestClient"))
