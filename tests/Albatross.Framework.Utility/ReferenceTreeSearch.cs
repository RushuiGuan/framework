...
namespace Albatross.Framework.Utility {
	...
	public class ReferenceTreeSearch : UtilityBase<ReferenceTreeSearchOptions> {
		...
		public IEnumerable<string> AsssemblyDirectories { get; private set; } = new string[0];
		...
		public bool TryFindAssemblyFile(AssemblyName assemblyReference, [NotNullWhen(true)]out FileInfo? file) {
		...
		public void Search(Stack<Assembly> stack, Assembly assembly) {
			...
			if (IsMatch(assembly)) {
				...
				foreach (var assemblyReference in assembly.GetReferencedAssemblies()) {
					if (TryFindAssemblyFile(assemblyReference, out var file)) {
						try {
