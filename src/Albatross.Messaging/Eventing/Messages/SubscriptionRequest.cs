...
namespace Albatross.Messaging.Eventing.Messages {
	public record class SubscriptionRequest : Message, IMessage {
		...
		public static IMessage Accept(string route, ulong id, NetMQMessage frames) {
			...
			return new SubscriptionRequest(route, id, on, isRegex, topic);
		}

		public SubscriptionRequest(string route, ulong id, bool on, bool isRegex, string topic) : base(MessageHeader, route, id) {
			this.On = on;
